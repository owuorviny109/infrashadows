...
import PowerLoadMetrics from './PowerLoadMetrics';

/**
 * Component for calculating power load based on building specifications
 */
function PowerLoadCalculator({ initialBuildingData = null }) {
  ...
  const calculatePowerLoad = () => {
    setCalculationComplete(true);
  };
  
  // Common amenities for power load calculation
  ...
  return (
    <div className="space-y-6">
      <Card title="Power Load Calculator">
        <div className="space-y-4">
          ...
          {/* Calculate button */}
          <div className="pt-2">
            <Button onClick={calculatePowerLoad} className="w-full">
              Calculate Power Load
            </Button>
          </div>
        </div>
      ...
      {/* Results */}
      {calculationComplete && (
        <PowerLoadMetrics buildingData={buildingData} />
      )}
    </div>
  );
}

export default PowerLoadCalculator;